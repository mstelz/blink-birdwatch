...
def start_publisher(*, fifo_path: Path, rtsp_url: str, transport: str) -> subprocess.Popen:
    ...
    # Re-mux video, but normalize AAC for RTSP so each new producer segment can hand off cleanly.
    cmd = ffmpeg_base() + [
        "-fflags",
...
def start_clip_to_pipe(src: Path, fifo_path: Path) -> subprocess.Popen:
    # Normalize into MPEG-TS/H264/AAC and aggressively repeat headers so a long-lived
